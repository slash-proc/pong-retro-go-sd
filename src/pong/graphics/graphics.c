...
#ifndef HOST_BUILD
	/* Firmware DMA2D R2M (same peripheral HAL handle as M2M). */
	if (dma2d_r2m_rgb565_start(ColorIndex, (uint32_t)pDst,
				   (uint16_t)xSize, (uint16_t)ySize,
				   (uint16_t)OffLine) == 0) {
		for (;;) {
			uint32_t st = dma2d_poll(50);
			wdog_refresh();
			if (st == HAL_OK)
				return;
			if (st != HAL_TIMEOUT)
				break; /* fall back to CPU fill */
		}
	}
#endif
